...
import com.quicktvui.support.lottie.R;
...
import quicktvui.support.lottie.animation.AnimatorLister;
...
import quicktvui.support.lottie.value.LottieFrameInfo;
import quicktvui.support.lottie.value.LottieValueCallback;
import quicktvui.support.lottie.value.SimpleLottieValueCallback;
...
public class LottieAnimationView extends ImageView implements IEsComponentView {
    ...
    private boolean isAnimationListenerOpen = false;
    private boolean isAnimationUpdateState = false;
    ...
    private Animator.AnimatorPauseListener mAnimatorPauseListener = null;
    ...
    private void init(@Nullable AttributeSet attrs, @AttrRes int defStyleAttr) {
        ...
        if (ta.hasValue(R.styleable.LottieAnimationView_lottie_renderMode)) {
            ...
            dealRenderMode(renderModeOrdinal);
        }
        ...
        if (ta.hasValue(R.styleable.LottieAnimationView_lottie_asyncUpdates)) {
            ...
            dealUpdatesOrdinal(asyncUpdatesOrdinal);
        }
    ...
    private void initListener(LottieAnimationView view) {
        mAnimatorListener = new AnimatorLister() {
            @Override
            public void onAnimationStart(Animator animation) {
                if (isAnimationListenerOpen) {
                    HippyMap params = new HippyMap();
                    params.pushString("eventName", "onAnimationStart");
                    HippyViewEvent event = new HippyViewEvent("onAnimationEvent");
                    event.send(view, params);
                }
            }
            ...
            public void onAnimationEnd(Animator animation) {
                if (isAnimationListenerOpen) {
                    HippyMap params = new HippyMap();
                    params.pushString("eventName", "onAnimationEnd");
                    params.pushString("params", animation.toString());
                    HippyViewEvent event = new HippyViewEvent("onAnimationEvent");
                    event.send(view, params);
                }
            }
            ...
            public void onAnimationCancel(Animator animation) {
                if (isAnimationListenerOpen) {
                    HippyMap params = new HippyMap();
                    params.pushString("eventName", "onAnimationCancel");
                    params.pushString("params", animation.toString());
                    HippyViewEvent event = new HippyViewEvent("onAnimationEvent");
                    event.send(view, params);
                }
            ...
            public void onAnimationRepeat(Animator animation) {
                if (isAnimationListenerOpen) {
                    HippyMap params = new HippyMap();
                    params.pushString("eventName", "onAnimationRepeat");
                    params.pushString("params", animation.toString());
                    HippyViewEvent event = new HippyViewEvent("onAnimationEvent");
                    event.send(view, params);
                }
        ...
        lottieDrawable.addAnimatorListener(mAnimatorListener);
        mAnimatorUpdateListener = new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                if (isAnimationUpdateState) {
                    HippyMap params = new HippyMap();
                    params.pushString("eventName", "onAnimationUpdate");
                    params.pushDouble("value", animation.getAnimatedFraction());
                    HippyViewEvent event = new HippyViewEvent("onAnimationEvent");
                    event.send(view, params);
                }
            }
        };
        lottieDrawable.addAnimatorUpdateListener(mAnimatorUpdateListener);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            mAnimatorPauseListener = new Animator.AnimatorPauseListener() {
                @Override
                public void onAnimationPause(Animator animation) {
                    if (isAnimationListenerOpen) {
                        HippyMap params = new HippyMap();
                        params.pushString("eventName", "onAnimationPause");
                        params.pushString("params", animation.toString());
                        HippyViewEvent event = new HippyViewEvent("onAnimationEvent");
                        event.send(view, params);
                    }
                ...
                public void onAnimationResume(Animator animation) {
                    if (isAnimationListenerOpen) {
                        HippyMap params = new HippyMap();
                        params.pushString("eventName", "onAnimationResume");
                        params.pushString("params", animation.toString());
                        HippyViewEvent event = new HippyViewEvent("onAnimationEvent");
                        event.send(view, params);
                    }
            ...
            lottieDrawable.addAnimatorPauseListener(mAnimatorPauseListener);
        }

    }

    public void dealUpdatesOrdinal(int asyncUpdatesOrdinal) {
        int asyncUpdatesCode = asyncUpdatesOrdinal;
        if (asyncUpdatesCode >= RenderMode.values().length) {
            asyncUpdatesCode = AsyncUpdates.AUTOMATIC.ordinal();
        }
        setAsyncUpdates(AsyncUpdates.values()[asyncUpdatesCode]);
    }

    public void dealRenderMode(int renderModeOrdinal) {
        int renderCode = renderModeOrdinal;
        if (renderCode >= RenderMode.values().length) {
            renderCode = RenderMode.AUTOMATIC.ordinal();
        }
        setRenderMode(RenderMode.values()[renderCode]);
    }

    public void dealColorFunction(int colorRes) {
        //ColorStateList csl = AppCompatResources.getColorStateList(getContext(), colorRes);
        SimpleColorFilter filter = new SimpleColorFilter(colorRes);
        KeyPath keyPath = new KeyPath("**");
        LottieValueCallback<ColorFilter> callback = new LottieValueCallback<>(filter);
        addValueCallback(keyPath, LottieProperty.COLOR_FILTER, callback);
    }

    public void setLottieLoop(boolean isLoop) {
        if (lottieDrawable != null && isLoop) {
            lottieDrawable.setRepeatCount(LottieDrawable.INFINITE);
        }
    }
    ...
    public void setAnimationFromUrl(String url) {
        LottieTask<LottieComposition> task = cacheComposition ? LottieCompositionFactory.fromUrl(getContext(), url) : LottieCompositionFactory.fromUrl(getContext(), url, null);
    ...
    public void setProgressInternal(
    ...
    public void setAnimationListenerState(boolean isOpen) {
        isAnimationListenerOpen = isOpen;
    }

    public void setAnimationUpdateState(boolean isOpen) {
        isAnimationUpdateState = isOpen;
    }
