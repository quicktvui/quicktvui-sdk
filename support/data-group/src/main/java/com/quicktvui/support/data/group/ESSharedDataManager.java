...
public class ESSharedDataManager {
    ...
    public boolean getBoolean(String selfPackageName, String sharedPackageName, String secretKey, String key, boolean defValue) {
        if (TextUtils.isEmpty(selfPackageName) || TextUtils.isEmpty(sharedPackageName)) {
            return defValue;
        }
        try {
            ESGroupData sharedData = getSharedData(sharedPackageName, key);
            if (sharedData != null && sharedData.getData() instanceof Boolean) {
                ...
                //
                else if (sharedData.getMode() >= ESGroupMode.MODE_WORLD_READABLE) {
                    String dataSecretKey = sharedData.getSecretKey();
                    if (TextUtils.isEmpty(dataSecretKey)) {
                        if (L.DEBUG) {
                            L.logD("#------getBoolean-------数据秘钥为空，可以访问-------------->>>");
                        }
                        return (Boolean) sharedData.getData();
                    } else if (!TextUtils.isEmpty(dataSecretKey) && dataSecretKey.equals(secretKey)) {
                        if (L.DEBUG) {
    ...
    public int getInt(String selfPackageName, String sharedPackageName, String secretKey, String key, int defValue) {
        if (TextUtils.isEmpty(selfPackageName) || TextUtils.isEmpty(sharedPackageName)) {
            return defValue;
        }
        try {
    ...
    public long getLong(String selfPackageName, String sharedPackageName, String secretKey, String key, long defValue) {
        if (TextUtils.isEmpty(selfPackageName) || TextUtils.isEmpty(sharedPackageName)) {
            return defValue;
        }
        try {
    ...
    public String getString(String selfPackageName, String sharedPackageName, String secretKey, String key, String defValue) {
        if (TextUtils.isEmpty(selfPackageName) || TextUtils.isEmpty(sharedPackageName)) {
            return defValue;
        }
        try {
    ...
    public EsArray getArray(String selfPackageName, String sharedPackageName, String secretKey, String key, EsArray defValue) {
        if (TextUtils.isEmpty(selfPackageName) || TextUtils.isEmpty(sharedPackageName)) {
            return defValue;
        }
    ...
    public EsMap getMap(String selfPackageName, String sharedPackageName, String secretKey, String key, EsMap defValue) {
        if (TextUtils.isEmpty(selfPackageName) || TextUtils.isEmpty(sharedPackageName)) {
            return defValue;
        }
        try {
    ...
    public boolean putBoolean(String selfPackageName, String sharedPackageName, String secretKey, String key, boolean value, int mode) {
        return putSharedData(selfPackageName, sharedPackageName, secretKey, key, value, ESGroupDataType.ES_SHARED_DATA_TYPE_BOOLEAN, mode);
    }

    public boolean putInt(String selfPackageName, String sharedPackageName, String secretKey, String key, int value, int mode) {
        return putSharedData(selfPackageName, sharedPackageName, secretKey, key, value, ESGroupDataType.ES_SHARED_DATA_TYPE_INT, mode);
    }

    public boolean putLong(String selfPackageName, String sharedPackageName, String secretKey, String key, long value, int mode) {
        return putSharedData(selfPackageName, sharedPackageName, secretKey, key, value + "", ESGroupDataType.ES_SHARED_DATA_TYPE_LONG, mode);
    }

    public boolean putString(String selfPackageName, String sharedPackageName, String secretKey, String key, String value, int mode) {
        return putSharedData(selfPackageName, sharedPackageName, secretKey, key, value, ESGroupDataType.ES_SHARED_DATA_TYPE_STRING, mode);
    }

    public boolean putArray(String selfPackageName, String sharedPackageName, String secretKey, String key, EsArray value, int mode) {
        return putSharedData(selfPackageName, sharedPackageName, secretKey, key, value, ESGroupDataType.ES_SHARED_DATA_TYPE_ARRAY, mode);
    }

    public boolean putMap(String selfPackageName, String sharedPackageName, String secretKey, String key, EsMap value, int mode) {
        return putSharedData(selfPackageName, sharedPackageName, secretKey, key, value, ESGroupDataType.ES_SHARED_DATA_TYPE_MAP, mode);
    }
    ...
    private ESGroupData getSharedData(String sharedPackageName, String key) {
        if (L.DEBUG) {
        ...
        sharedPreferencesManager.initSharedPreferences(sharedPackageName + "_group_data");
        String sharedDataString = sharedPreferencesManager.getString(key, null);
        if (L.DEBUG) {
            L.logD("#-----------getSharedData-----string------>>>" + sharedDataString);
        }
        ESGroupData sharedData = stringToSharedData(sharedDataString);
        if (L.DEBUG) {
    ...
    public boolean putSharedData(String selfPackageName, String sharedPackageName, //
        ...
        if (isSelfData(selfPackageName, sharedPackageName)) {
            ...
            sharedData.setSecretKey(secretKey);//自己改秘钥
            saveSharedData(sharedPreferencesManager, key, sharedData);
            return true;
        }
        //
        else {
            if (sharedData.getMode() >= ESGroupMode.MODE_WORLD_WRITEABLE) {
                if (L.DEBUG) {
    ...
    private void saveSharedData(AndroidSharedPreferencesManager preferencesManager, //
                                String key, ESGroupData sharedData) {
        try {
    ...
    private ESGroupData stringToSharedData(String json) {
        ...
        try {
            ...
            sharedData.setSecretKey(jsonObject.optString("secretKey"));
    ...
    private String sharedDataToString(ESGroupData sharedData) {
        try {
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("mode", sharedData.getMode());
            jsonObject.put("type", sharedData.getType());
            jsonObject.put("secretKey", sharedData.getSecretKey());
