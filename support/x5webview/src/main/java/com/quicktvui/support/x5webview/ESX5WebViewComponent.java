...
/** X5浏览器 **/
@ESKitAutoRegister
public class ESX5WebViewComponent implements IEsComponent<ESX5WebView> {

    private static final String TAG = ESX5WebViewComponent.class.getSimpleName();

    public static final String CLASS_NAME = "TVX5WebView";
    ...
    private static final String OP_INIT_JAVA_SCRIPT_INTERFACE = "initJavaScriptInterface";//
    private static final String OP_REMOVE_JAVA_SCRIPT_INTERFACE = "removeJavaScriptInterface";//

    private static final String OP_INIT_WEBVIEW_FOCUS = "initWebViewFocus";//

    protected Context context;

    @Override
    public ESX5WebView createView(Context context, EsMap params) {
        this.context = context;
        return new ESX5WebView(context);
    }

    @EsComponentAttribute
    public void url(ESX5WebView webView, String url) {
    ...
    @EsComponentAttribute
    public void openSimulateClick(ESX5WebView webView, boolean open) {
        if (webView != null) {
            webView.openSimulateClick(open);
        }
    }


    @Override
    public void dispatchFunction(ESX5WebView webView, String eventName, EsArray params, EsPromise promise) {
        if (L.DEBUG) {
        ...
        try {
            switch (eventName) {
                ...
                case OP_SET_APP_CACHE_ENABLED:
                    try {
                        ...
                        if (webView != null && webView.getSettings() != null) {
                            ...
                            webView.getSettings().setAppCacheEnabled(enabled);
                        }
                    } catch (Throwable e) {
                        e.printStackTrace();
                    }
                ...
                case OP_SET_APP_CACHE_MAX_SIZE:
                    try {
                        ...
                        if (webView != null && webView.getSettings() != null) {
                            ...
                            webView.getSettings().setAppCacheMaxSize(value);
                        }
                    } catch (Throwable e) {
                        e.printStackTrace();
                    }
                ...
                case OP_SET_SAFE_BROWSING_ENABLED:
                    ...
                    break;
                case OP_INIT_JAVA_SCRIPT_INTERFACE:
                    try {
                        if (webView != null) {
                            if (L.DEBUG) {
                                L.logD("#---initJavaScriptInterface------>>>");
                            }
                            webView.initJavaScriptInterface();
                        }
                    } catch (Throwable e) {
                        e.printStackTrace();
                    }
                    break;
                case OP_REMOVE_JAVA_SCRIPT_INTERFACE:
                    try {
                        if (webView != null) {
                            if (L.DEBUG) {
                                L.logD("#---removeJavaScriptInterface------>>>");
                            }
                    ...
                    break;
                case OP_INIT_WEBVIEW_FOCUS:
                    try {
                        long time = params.getLong(0);
                        int x = params.getInt(1);
                        int y = params.getInt(2);
                        if (webView != null) {
                            if (L.DEBUG) {
                                L.logD("#---initWebViewFocus------>>>");
                            }
                            webView.initWebViewFocus(time, x, y);
                        }
                    } catch (Throwable e) {
                        e.printStackTrace();
                    }
    ...
    @Override
    public void destroy(ESX5WebView view) {
