...
import com.quicktvui.support.canvas.annotation.CanvasMethod;
...
public class CanvasContextRendering2D extends CanvasContext {
    ...
    private final Stack<CanvasContextState> mStates = new Stack<>();
    private Paint mCompositePaint;
    private Canvas mSaveCountCanvas;
    private final Matrix mIdentifyMatrix = new Matrix();
    ...
    private OnPicFinishListener mPicListener;
    ...
    public void setPicListener(OnPicFinishListener mPicListener) {
    ...
    public void setSupportHardWare(boolean isSupportHare) {
        this.hasHardWare = isSupportHare;
    }
    ...
    public @NonNull
    CanvasContextState currentState() {
        return mState;
    }
    ...
    public void render(CanvasView2D canvasView, Canvas canvas, ArrayList<CanvasRenderAction> actions) {
    ...
    public void setFillStyle(CanvasGradient canvasGradient) {
        ...
        float width = DisplayUtil.getScreenWidth(Utils.getApp());
    ...
    public void setFillStyle(@NonNull Bitmap bitmap, String pattern) {
        ...
        if (bitmap != mState.mFillPatternOriginBitmap || !TextUtils.equals(mState.mFillPattern, pattern) || mState.mFillPatternBitmap == null) {
            mState.recycleFillPattern();
            if (hasHardWare) {
                ...
                mState.mFillPatternOriginBitmap = softBitmap;
                mState.mFillPatternBitmap = resolvePatternBitmap(softBitmap, xRepeat, yRepeat);
            }
        }
    ...
    public void createCachedBitmap() {
    ...
    public void createCompositeBitmap() {
        if (mCurrentCanvas == null) {
            return;
        }

        if (mDstBitmap == null || mDstBitmap.getWidth() != mCurrentCanvas.getWidth()
                || mDstBitmap.getHeight() != mCurrentCanvas.getHeight()) {
            recycleDstBitmap();
            mDstBitmap = createBitmap(mCurrentCanvas.getWidth(), mCurrentCanvas.getHeight(), Bitmap.Config.ARGB_8888);
            if (mDstBitmap != null) {
        ...
        if (mSrcBitmap == null || mSrcBitmap.getWidth() != mCurrentCanvas.getWidth()
                || mSrcBitmap.getHeight() != mCurrentCanvas.getHeight()) {
            recycleSrcBitmap();
            mSrcBitmap = createBitmap(mCurrentCanvas.getWidth(), mCurrentCanvas.getHeight(), Bitmap.Config.ARGB_8888);
            if (mSrcBitmap != null) {
    ...
    public void drawImage(
        ...
        /*if (dx <= 0 && dy <= 0 && dstWidth <= 0) {
            mState.mFillPaint.setFilterBitmap(true);
            mState.mFillPaint.setDither(true);
            mCurrentCanvas.drawBitmap(bitmap, sx, sy, mState.mFillPaint);
            mState.mFillPaint.setShadowLayer(mState.mShadowBlur, mState.mShadowOffsetX, mState.mShadowOffsetY, mState.mShadowColor);
            return;
        }
        if (sx > 0 && sy > 0) {
            mState.mFillPaint.setFilterBitmap(true);
            mState.mFillPaint.setDither(true);
            mCurrentCanvas.drawBitmap(bitmap, sx + srcWidth, sy + srcHeight, mState.mFillPaint);
            mState.mFillPaint.setShadowLayer(mState.mShadowBlur, mState.mShadowOffsetX, mState.mShadowOffsetY, mState.mShadowColor);
            return;
        }*/
        Rect srcRect;
        ...
        if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.P) {
            srcRect = new Rect(sx < 0 ? 0 : (int) sx,   sy < 0 ? 0 : (int) sy,
                    sx + srcWidth < bitmap.getWidth() ? (int) (sx + srcWidth) : bitmap.getWidth(),
                    sy + srcHeight < bitmap.getHeight() ? (int) (sy + srcHeight) : bitmap.getHeight());
            float left = dx;
            float top = dy;
            ...
            // 强制限制在 Bitmap 范围内
            rectRight = Math.min(bitmap.getWidth(), (int) (sx + srcWidth));
            RectBottom = Math.min(bitmap.getHeight(), (int) (sy + srcHeight));
            srcRect = new Rect((int) sx, (int) sy, rectRight, RectBottom);
        ...
        mBitmapPaint.setShadowLayer(0, 0, 0, Color.TRANSPARENT);
        if (hasHardWare) {//安卓o以上
            mCurrentCanvas.drawBitmap(bitmap, srcRect, dstRect, mBitmapPaint);
            mBitmapPaint.setShadowLayer(mState.mShadowBlur, mState.mShadowOffsetX, mState.mShadowOffsetY, mState.mShadowColor);
        } else {
            if (dx <= 0 && dy <= 0 && dstWidth <= 0) {
                mState.mFillPaint.setFilterBitmap(true);
                mState.mFillPaint.setDither(true);
...
//            Bitmap softwareBitmap = bitmap.copy(Bitmap.Config.ARGB_4444, true);
            mCurrentCanvas.drawBitmap(bitmap, srcRect, dstRect, mBitmapPaint);
            mState.mFillPaint.setShadowLayer(mState.mShadowBlur, mState.mShadowOffsetX, mState.mShadowOffsetY, mState.mShadowColor);
        }
        ...
        /*ThreadUtils.runOnUiThread(() -> {
            if (mCanvasView != null) {
                mCanvasView.invalidate(srcRect);
            }
        });*/
    }
    ...
    /*@CanvasMethod
    public void fillRect(Canvas canvas, float x, float y, float width, float height) {
        if (mCurrentCanvas == null) {
            return;
        }
        x = getRealSize(x);
        ...
        if (isNeedGlobal) {
            canvas.drawBitmap(mSrcBitmap, 0, 0, mCompositePaint);
            mSrcBitmap.eraseColor(Color.TRANSPARENT);

            if (mDstBitmap != null && mCurrentCanvas != null) {
                canvas.drawBitmap(mDstBitmap, 0, 0, null);
            }
            isNeedGlobal = false;
        } else {
    ...
    public Bitmap dumpBitmap() {
        ...
        Bitmap bitmap;
        int canvasWidth = mCurrentCanvas.getWidth();
        int canvasHeight = mCurrentCanvas.getHeight();
        ...
        Bitmap image = createBitmap(bitmap, 0, 0, bitmap.getWidth(), bitmap.getHeight(), matrix, true);
        bitmap.recycle();
        return image;
    }
    ...
    public void putImageData(Bitmap bitmap, float dx, float dy) {
        ...
        mCurrentCanvas.drawBitmap(bitmap, null, new RectF(dx, dy, dx + width, dy + height), mState.mFillPaint);
        mState.mFillPaint.setShadowLayer(mState.mShadowBlur,
                mState.mShadowOffsetX, mState.mShadowOffsetY, mState.mShadowColor);
    }
     ...
     */
    @CanvasMethod
    public void transform(float scaleX, float skewY, float skewX, float scaleY, float translateX, float translateY) {
        if (mCurrentCanvas == null) {
            return;
        }
        Matrix matrix = new Matrix();
