...
package com.quicktvui.support.canvas.image;
...
import com.quicktvui.support.canvas.bridge.Callback;
import com.quicktvui.support.canvas.bridge.Response;
import com.quicktvui.support.canvas.executors.Executors;
import com.quicktvui.support.canvas.utils.ThreadUtils;
import com.quicktvui.support.canvas.utils.BitmapUtils;
import com.quicktvui.support.canvas.utils.DigestUtils;
...
public class CanvasImageHelper implements CanvasImageLoader, ComponentCallbacks {
    ...
    public void loadImage(Uri uri, Object imageId, Callback callback) {
        ...
        if (uri == null || imageId == null) {
            result.put("error", "invalid param. specify an json array which length is 2, and index 0 is url to load, index 1 is image id.");
                ...
                .execute(
                        () -> {
                            ...
                            if (src.startsWith(SCHEMA_SHA)) {
                                ...
                                /*BitmapUtils.fetchBitmap(Uri.parse(src), new BitmapUtils.BitmapLoadCallback() {
                                            @Override
    ...
    private Bitmap handleBase64Texture(String url) {
        try {
            ...
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.KITKAT) {
                decodedBytes = Base64.decode(url.getBytes(StandardCharsets.UTF_8), Base64.DEFAULT);
            }
    ...
    private String buildKey(String content) {
        String md5 = null;
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.KITKAT) {
            md5 = DigestUtils.getSha256(content.getBytes(StandardCharsets.UTF_8));
        }
