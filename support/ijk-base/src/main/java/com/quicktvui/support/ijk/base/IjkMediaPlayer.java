...
import com.quicktvui.support.ijk.base.annotations.AccessedByNative;
import com.quicktvui.support.ijk.base.annotations.CalledByNative;
import com.quicktvui.support.ijk.base.misc.IAndroidIO;
...
import com.quicktvui.support.ijk.base.pragma.DebugLog;
...
public final class IjkMediaPlayer extends AbstractMediaPlayer {
    ...
    private static final int MEDIA_BITMAP_SUBTITLE = 98;
    ...
    public static void loadLibrariesOnce(IjkLibLoader libLoader) {
        synchronized (IjkMediaPlayer.class) {
            if (!mIsLibLoaded) {
                ...
                libLoader.loadLibrary("extffmpeg");
                libLoader.loadLibrary("extsdl");
                libLoader.loadLibrary("extplayer");
                mIsLibLoaded = true;
            }
        }
    }
    ...
    static boolean haveNativeSubSource = true;
    public void setSubDataSource(String path) {
        if (path != null && haveNativeSubSource) {
            try {
                _setSubDataSource(path);
            } catch (Throwable e) {
                haveNativeSubSource = false;
            }
        }
    }
    public void closeTimedFile() {
        if (!haveNativeSubSource) return;
        try {
            _closeTimedFile();
        } catch (Throwable e) {
            haveNativeSubSource = false;
    ...
    @CalledByNative
    private static String getExtraNativeMethodList() {
        return "_setSubDataSource,_closeTimedFile";
    }

    private native void _setSubDataSource(String path)
            throws IOException, IllegalArgumentException, SecurityException, IllegalStateException;

    private native void _closeTimedFile();
    ...
    public static int TIMED_CLOSE = 0;
    public static int TIMED_ONLY_TEXT = 1;
    public static int TIMED_OPEN = 2;
    public static int TIMED_ONLY_BITMAP = 3;
    public static int TIMED_BITMAP = 0;
    public static int TIMED_TEXT = 1;
    public IjkTrackInfo[] getTrackInfoSubFilter(int timedType) {
        ...
        for (IjkMediaMeta.IjkStreamMeta streamMeta: mediaMeta.mStreams) {
            ...
            if (streamMeta.mType.equalsIgnoreCase(IjkMediaMeta.IJKM_VAL_TYPE__VIDEO)) {
                ...
                // 字幕类型过滤
                if (streamMeta.mSubType == TIMED_BITMAP && timedType > TIMED_ONLY_TEXT) {
                    trackInfo.setTrackType(ITrackInfo.MEDIA_TRACK_TYPE_TIMEDTEXT);
                } else if (streamMeta.mSubType == TIMED_TEXT &&
                        (timedType > TIMED_CLOSE && timedType < IjkMediaPlayer.TIMED_ONLY_BITMAP)) {
                    trackInfo.setTrackType(ITrackInfo.MEDIA_TRACK_TYPE_TIMEDTEXT);
                }
            }
    ...
    public MediaInfo getMediaInfo() {
        ...
        mediaInfo.mMediaPlayerName = getClass().getSimpleName();
    ...
    private static class EventHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            case MEDIA_BITMAP_SUBTITLE:
                if (msg.obj == null) {
//                    player.notifyOnTimedText(null);
                    player.notifyOnTimedBitmap(new IjkTimedBitmap(null));
                } else {
...
//                    player.notifyOnTimedText(subtitle);
                    player.notifyOnTimedBitmap(new IjkTimedBitmap(bitmap, bitmapWidth, bitmapHeight));
                }
            break;
