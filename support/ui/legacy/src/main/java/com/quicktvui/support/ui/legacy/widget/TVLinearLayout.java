...
public class TVLinearLayout extends LinearLayout implements ITVViewGroup, ITVFocusGroup {

    private final String TAG = "ScaleGroup";

    public static boolean DEBUG = FConfig.DEBUG;

    private int duration = TVFocusScaleExcuter.DEFAULT_DURATION;
    ...
    public TVLinearLayout(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    protected void onInitializeFromAttributes( AttributeSet attrs){
        if(attrs != null) {
            ...
            TypedArray typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.TVLinearLayout);

            setFocusScale(typedArray.getFloat(R.styleable.TVLinearLayout_value_focus_scale, TVFocusScaleExcuter.DEFAULT_SCALE));

            setFocusScaleX(typedArray.getFloat(R.styleable.TVLinearLayout_value_focus_scaleX,mFocusScaleX));
            setFocusScaleY(typedArray.getFloat(R.styleable.TVLinearLayout_value_focus_scaleY,mFocusScaleY));

            setupFloatFocusMargin(typedArray);

            setFloatFocusFocusedAlpha(typedArray.getFloat(R.styleable.TVLinearLayout_float_focus_focused_alpha,1));

            setFocusScaleDuration(typedArray.getInt(R.styleable.TVLinearLayout_duration_focus_scale, TVFocusScaleExcuter.DEFAULT_DURATION));
...
//        int left = typedArray.getDimensionPixelSize(R.styleable.FLinearLayout_float_focus_marginLeft,0);
//        int right = typedArray.getDimensionPixelSize(R.styleable.FLinearLayout_float_focus_marginRight,0);
//        int top = typedArray.getDimensionPixelSize(R.styleable.FLinearLayout_float_focus_marginTop,0);
//        int bottom = typedArray.getDimensionPixelSize(R.styleable.FLinearLayout_float_focus_marginBottom,0);

        int left = ConvertUtil.convertPixel(typedArray, R.styleable.TVLinearLayout_float_focus_marginLeft, 0);
        int right = ConvertUtil.convertPixel(typedArray, R.styleable.TVLinearLayout_float_focus_marginRight, 0);
        int top = ConvertUtil.convertPixel(typedArray, R.styleable.TVLinearLayout_float_focus_marginTop, 0);
        int bottom = ConvertUtil.convertPixel(typedArray, R.styleable.TVLinearLayout_float_focus_marginBottom, 0);
