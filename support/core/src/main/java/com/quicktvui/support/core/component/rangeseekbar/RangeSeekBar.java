...
import static com.quicktvui.support.core.component.rangeseekbar.SeekBar.INDICATOR_ALWAYS_HIDE;
import static com.quicktvui.support.core.component.rangeseekbar.SeekBar.INDICATOR_ALWAYS_SHOW;
...
public class RangeSeekBar extends View implements IEsComponentView {
    ...
    private Drawable progressDrawableId;
    //the default Drawable of the progress bar
    private Drawable progressDefaultDrawableId;

    //the url of seekBar in progress
    private String progressUrl;
    //the default url of the progress bar
    private String progressDefaultUrl;
    ...
    List<Bitmap> stepsBitmaps = Collections.synchronizedList(new ArrayList<>());
    private int progressPaddingRight;
    private OnRangeChangedListener callback;
    private OnRangeFocusListener onRangeFocusListener;
    ...
    private void initProgressBitmap() {
        if (progressDrawableId != null) {
            ...
            if (L.DEBUG) {
                L.logD("initProgressBitmap==>"//
                        + "progressWidth:" + progressWidth + "\n"//
                        + "progressHeight:" + progressHeight + "\n"//
                        + "progressDrawableId:" + progressDrawableId + "\n"//
                        + "progressBitmapWidth:" + (progressBitmap != null ? progressBitmap.getWidth() : "-1") + "\n"//
                        + "progressBitmapHeight:" + (progressBitmap != null ? progressBitmap.getHeight() : "-1"));
            }
        }
        if (progressDefaultDrawableId != null) {
            progressDefaultBitmap = Utils.drawableToBitmap(getContext(), progressWidth, progressHeight, progressDefaultDrawableId);
            if (L.DEBUG) {
                L.logD("initProgressBitmap==>"//
                        + "progressWidth:" + progressWidth + "\n"//
                        + "progressHeight:" + progressHeight + "\n"//
                        + "progressDefaultDrawableId:" + progressDefaultDrawableId + "\n"//
                        + "progressDefaultBitmapWidth:" + (progressDefaultBitmap != null ? progressDefaultBitmap.getWidth() : "-1") + "\n"//
                        + "progressDefaultBitmapHeight:" + (progressDefaultBitmap != null ? progressDefaultBitmap.getHeight() : "-1"));
            }
        }
    }

    private void initProgressUrlBitmap(IEsTraceable traceable) {
        if (progressBitmap == null && !TextUtils.isEmpty(progressUrl)) {
            EsMap data = new EsMap();
            data.pushString("url", progressUrl);
            data.pushInt("width", progressWidth);
            data.pushInt("height", progressHeight);
        ...
        if (progressDefaultBitmap == null && !TextUtils.isEmpty(progressDefaultUrl)) {
            EsMap data = new EsMap();
            data.pushString("url", progressDefaultUrl);
            data.pushInt("width", progressWidth);
            data.pushInt("height", progressHeight);
            EsProxy.get().loadImageBitmap(data, new EsCallback<Bitmap, Throwable>() {
                @Override
                public void onSuccess(Bitmap bitmap) {
                    progressDefaultBitmap = bitmap;
                }
    ...
    protected void onFocusChanged(boolean gainFocus, int direction, @Nullable Rect previouslyFocusedRect) {
        ...
        try {
            if (onRangeFocusListener != null) {
                onRangeFocusListener.onRangeFocusChange(this, gainFocus);
            }
        } catch (Throwable e) {
            e.printStackTrace();
        }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        onSeekBarSizeChanged(w, h, oldw, oldh);
    }

    protected void onSeekBarSizeChanged(int w, int h, int oldw, int oldh) {
    ...
    protected void onDrawSteps(Canvas canvas, Paint paint) {
        ...
        if (isLoggable) {
            Log.e(TAG, "-------onDrawSteps-------->>>>" + stepMarks);
        }
    ...
    public void setOnRangeFocusListener(OnRangeFocusListener listener) {
        onRangeFocusListener = listener;
    }
    ...
    public void setProgress(float leftValue, float rightValue) {
        ...
        if (leftValue < minProgress) {
            L.logW("setProgress() min < (preset min - offsetValue) . #min:" + leftValue + " #preset min:" + rightValue);
            return;
        }
        if (rightValue > maxProgress) {
            L.logW("setProgress() max > (preset max - offsetValue) . #max:" + rightValue + " #preset max:" + rightValue);
            return;
        }
    ...
    public void setRange(float min, float max, float minInterval) {
        if (max <= min) {
            L.logW("setRange() max must be greater than min ! #max:" + max + " #min:" + min);
            return;
        }
        if (minInterval < 0) {
            L.logW("setRange() interval must be greater than zero ! #minInterval:" + minInterval);
            return;
        }
        if (minInterval >= max - min) {
            L.logW("setRange() interval must be less than (max - min) ! #minInterval:" + minInterval + " #max - min:" + (max - min));
            return;
        }
    ...
    public void setTickMarkTextArray(CharSequence[] tickMarkTextArray) {
        ...
        if (isLoggable) {
            Log.e(TAG, "----------setTickMarkTextArray---->>>>>" //
                    + "tickMarkTextArray:" + tickMarkTextArray + "\n"//
                    + "progressWidth:" + progressWidth + "\n");
        }
    }
    ...
    public void setProgressDrawableId(Drawable progressDrawableId) {
        this.progressDrawableId = progressDrawableId;
        if (L.DEBUG) {
            L.logD("setProgressDrawableId==>" + progressDrawableId);
        }
        progressBitmap = null;
        initProgressBitmap();
    }

    public void setProgressUrl(IEsTraceable traceable, String progressUrl) {
        this.progressUrl = progressUrl;
        progressBitmap = null;
        if (L.DEBUG) {
            L.logD("setProgressUrl==>" + progressUrl);
        }
        initProgressUrlBitmap(traceable);
    }

    public void setProgressDefaultUrl(IEsTraceable traceable, String progressDefaultUrl) {
        this.progressDefaultUrl = progressDefaultUrl;
        progressDefaultBitmap = null;
        if (L.DEBUG) {
            L.logD("setProgressDefaultUrl==>" + progressDefaultUrl);
        }
        initProgressUrlBitmap(traceable);
    }
    ...
    public void setProgressDefaultDrawableId(Drawable progressDefaultDrawableId) {
        this.progressDefaultDrawableId = progressDefaultDrawableId;
        if (L.DEBUG) {
            L.logD("setProgressDefaultDrawableId==>" + progressDefaultDrawableId);
        }
        progressDefaultBitmap = null;
        initProgressBitmap();
    }
    ...
    public void setGravity(@GravityDef int gravity) {
        ...
        if (L.DEBUG) {
            L.logD("setGravity==>" + gravity);
        }
    }

    public void invalidateSeekBar() {
        onSeekBarSizeChanged(getWidth(), getHeight(), getWidth(), getHeight());
    }
    ...
    public void setStepsBitmaps(List<Bitmap> stepsBitmaps) {
        if (isLoggable) {
            Log.e(TAG, "-----setStepsUrl---setStepsBitmaps----->>>>" + stepsBitmaps);
        }
        if (stepsBitmaps == null || stepsBitmaps.isEmpty() || stepsBitmaps.size() <= steps) {
            L.logW("stepsBitmaps must > steps !");
            return;
        }
        this.stepsBitmaps.clear();
    ...
    public void setStepsDrawableIds(List<Integer> stepsDrawableIds) {
        if (stepsDrawableIds == null || stepsDrawableIds.isEmpty() || stepsDrawableIds.size() <= steps) {
            L.logW("stepsDrawableIds must > steps !");
            return;
        }
        if (!verifyStepsMode()) {
            L.logW("stepsWidth must > 0, stepsHeight must > 0,steps must > 0 First!!");
            return;
        }
    ...
    public void setStepsDrawable(List<Drawable> stepsDrawableIds) {
        if (stepsDrawableIds == null || stepsDrawableIds.isEmpty() || stepsDrawableIds.size() <= steps) {
            L.logW("stepsDrawableIds must > steps !");
            return;
        }
    ...
    public void setStepsUrl(List<String> urls) {
        if (urls == null || urls.isEmpty() || urls.size() <= steps) {
            L.logW("stepsUrls must > steps !");
            return;
        }
        if (!verifyStepsMode()) {
            L.logW("stepsWidth must > 0, stepsHeight must > 0,steps must > 0 First!!");
        ...
        for (int i = 0; i < urls.size(); i++) {
            EsMap data = new EsMap();
            data.pushString("url", urls.get(i));
            data.pushInt("width", (int) stepsWidth);
            data.pushInt("height", (int) stepsHeight);
            EsProxy.get().loadImageBitmap(data, new EsCallback<Bitmap, Throwable>() {
                @Override
                public void onSuccess(Bitmap bitmap) {
                    count[0]++;
                    ...
                    if (count[0] >= (urls.size())) {
                        if (isLoggable) {
                            Log.e(TAG, urls.size() + "---setStepsUrl-loadStep----onSuccess---1-->>>>" + count[0]);
                        }
                        setStepsBitmaps(stepsBitmaps);
                        invalidate();
                    } else {
                        if (isLoggable) {
                            Log.e(TAG, urls.size() + "---setStepsUrl-loadStep----onSuccess--2--->>>>" + count[0]);
                        }
                    }
                }
                ...
                public void onFailed(Throwable throwable) {
                    if (isLoggable) {
                        Log.e(TAG, "---setStepsUrl-loadStep----onFailed----->>>>" + count[0]);
                    }
                    if (count[0] >= (urls.size())) {
                        setStepsBitmaps(stepsBitmaps);
                        invalidate();
